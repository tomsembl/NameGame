...
def get_user_inst_id(user_id,game_id):
    return query_sql(f"select user_inst_id from user_instance where game_id = {game_id} and user_id={user_id}")[0][0]

def get_game_deets(game_id):
    return header_zip_query(f"select * from games where game_id = {game_id}")
...
def emit_next_name(game_id,user_id):
    names = header_zip_query(f"select * from names where game_id = {game_id}",multi=True)
...
def add_game():
    ...
    query = f"insert into games ({keys}, date_created, active, stage) values ({values}, NOW(), TRUE, 1); select game_id from games order by game_id desc limit 1"
...
@app.route('/delete_game/<game_id>', methods=["GET"])
...
def join_game(game_id):
    if not game_id:
        ...
        return render_template('join-game.html',games=games_sql)
    game_deets = get_game_deets(game_id)
